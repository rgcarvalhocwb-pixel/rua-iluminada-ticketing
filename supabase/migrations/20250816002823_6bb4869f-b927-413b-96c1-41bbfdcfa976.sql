-- CRITICAL SECURITY FIX: Remove public access - use unique policy names
-- The issue is policies are using "public" role instead of "authenticated"

-- =============================================================================
-- FIX ORDERS TABLE - Remove public access (unique names)
-- =============================================================================
...
-- Create secure orders policies with unique names (authenticated only)
CREATE POLICY "Auth_Admin_View_Orders_2025" 
ON public.orders 
FOR SELECT 
TO authenticated
USING (has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'master'::app_role));

CREATE POLICY "Auth_Admin_Insert_Orders_2025" 
ON public.orders 
FOR INSERT 
TO authenticated
WITH CHECK (has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'master'::app_role));

CREATE POLICY "Auth_Admin_Update_Orders_2025" 
ON public.orders 
FOR UPDATE 
TO authenticated
USING (has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'master'::app_role))
...
CREATE POLICY "Auth_Admin_Delete_Orders_2025" 
ON public.orders 
FOR DELETE 
TO authenticated
USING (has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'master'::app_role));

CREATE POLICY "Auth_Terminal_Insert_Orders_2025" 
ON public.orders 
FOR INSERT 
TO authenticated
WITH CHECK (is_terminal_user(auth.uid()) AND (created_by = auth.uid()));

CREATE POLICY "Auth_Terminal_Update_Orders_2025" 
ON public.orders 
FOR UPDATE 
TO authenticated
...
CREATE POLICY "Auth_Terminal_View_Orders_2025" 
ON public.orders 
FOR SELECT 
TO authenticated
USING (is_terminal_user(auth.uid()) AND (created_by = auth.uid()));

CREATE POLICY "Auth_Customer_View_Orders_2025" 
ON public.orders 
FOR SELECT 
TO authenticated
...
-- =============================================================================
-- FIX PROFILES TABLE - Remove public access (unique names)
-- =============================================================================
...
-- Create secure profiles policies with unique names (authenticated only)
CREATE POLICY "Auth_Admin_Manage_Profiles_2025" 
ON public.profiles 
FOR ALL 
TO authenticated
USING (has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'master'::app_role))
...
CREATE POLICY "Auth_User_View_Own_Profile_2025" 
ON public.profiles 
FOR SELECT 
TO authenticated
USING (auth.uid() = user_id);

CREATE POLICY "Auth_User_Insert_Own_Profile_2025" 
ON public.profiles 
FOR INSERT 
TO authenticated
WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Auth_User_Update_Own_Profile_2025" 
ON public.profiles 
FOR UPDATE 
TO authenticated
USING (auth.uid() = user_id)
